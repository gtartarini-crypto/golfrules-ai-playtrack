...
interface RenderMainViewProps {
  ...
  confirmEndRound: () => Promise<void>; // lasciato per compatibilità
  popup: any;
  setPopup: (p: any) => void;
  // Added onStopRound property to interface
  onStopRound: () => void;
}
...
export const MainViewRenderer: React.FC<RenderMainViewProps> = (props) => {
  const {
    ...
    // Destructured onStopRound from props
    onStopRound
  } = props;
  ...
  useEffect(() => {
    ...
    if (user.role === 'player' && adminViews.includes(view)) {
      setView('player_home');
    }

    if (user.role !== 'sys_admin' && view === 'super_admin_dashboard') {
      setView(user.role === 'player' ? 'player_home' : 'club_dashboard');
    }
  }, [view, user, setView]);
  ...
  const handleAdminLogout = async () => {
    await logout();
    // Il reindirizzamento a 'landing' è gestito dal wrapper in App.tsx
  };
    ...
    <aside className="hidden lg:flex w-80 bg-slate-900 flex-col flex-shrink-0 shadow-2xl z-40 border-r border-white/5">
      <div className="p-10 border-b border-white/5 flex items-center gap-4">
        ...
        <div className="flex flex-col">
          ...
          <span className="text-[10px] font-black text-emerald-400 uppercase tracking-widest mt-1.5">
            Admin Management
          </span>
        </div>
      </div>
      ...
      <nav className="flex-1 p-6 space-y-1 overflow-y-auto scrollbar-hide">
        {menuItems.map((item: any, idx: number) => {
          if (item.type === 'separator') {
            return <div key={`sep-${idx}`} className="h-px bg-white/10 my-4 mx-2" />;
          }
          ...
          return (
            <button
              key={item.id}
              onClick={() => !locked && handleNavigate(item.id as AppView)}
              className={`w-full flex items-center justify-between p-4 rounded-[1rem] transition-all group ${
                isActive
                  ? 'bg-emerald-50 text-slate-950 shadow-lg shadow-emerald-500/20 border border-white/10'
                  : locked
            ...
            >
              <div className="flex items-center gap-4">
                {item.icon && (
                  <item.icon
                    size={20}
                    className={isActive ? 'text-slate-950' : 'text-slate-500 group-hover:text-white'}
                  />
                )}
      ...
      <div className="p-6 border-t border-white/5">
        <div className="flex items-center gap-4 p-4 bg-white/5 rounded-[1.25rem] border border-white/5">
          <div className="w-10 h-10 bg-emerald-500/10 rounded-xl flex items-center justify-center text-emerald-500 font-black border border-emerald-500/20">
  ...
  const renderMasterHeader = () => (
    <div className="flex flex-col z-30 shrink-0">
      {activeHealthAlerts.length > 0 && (
        <div
          onClick={() => handleNavigate('player_monitor', { flightId: activeHealthAlerts[0].flightId })}
          className="bg-red-600 animate-pulse text-white px-10 py-4 flex items-center justify-between cursor-pointer hover:bg-red-700 transition-colors border-b border-red-500 shadow-2xl"
        >
          <div className="flex items-center gap-4">
            <HeartPulse size={24} className="animate-bounce" />
            <div>
              <p className="text-xs font-black uppercase tracking-widest leading-none">
                SOS Emergenza Medica Attiva
              </p>
              <h3 className="text-lg font-black uppercase tracking-tighter">
                {activeHealthAlerts[0].userName} alla {activeHealthAlerts[0].areaContext}
              </h3>
            </div>
          </div>
          <button className="bg-white text-red-600 px-6 py-2 rounded-full font-black text-xs uppercase tracking-widest shadow-xl">
            Intervieni Ora
          </button>
        </div>
      )}
      ...
      <header className="bg-white border-b border-slate-200 px-10 py-6 flex items-center justify-between shadow-sm">
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-4 mt-2">
              <span className="flex items-center gap-1.5 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                <Globe size={12} className="text-emerald-500" /> {localRulesData?.country || '---'}
              </span>
              <div className="h-1 w-1 bg-slate-300 rounded-full" />
              <div
                className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] font-black uppercase ${
                  isPremium ? 'bg-purple-100 text-purple-700' : 'bg-slate-100 text-slate-500'
                }`}
              >
                {isPremium ? <ShieldCheck size={10} /> : <Clock size={10} />}
                {isPremium ? 'Account Premium Attivo' : 'Piano Base'}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6">
          ...
          <button
            onClick={() => setView('debug_dashboard')}
  ...
  const renderContent = () => {
    switch (view) {
      case 'landing':
        return (
          <LandingPage
            onStartScanner={() => setView('qr_scanner')}
            onLogin={handlePortalEntry}
            lang={lang}
            onToggleLang={setLang}
            user={user}
            onLogout={logout}
            clubName={localRulesData?.name || localRulesData?.club || ""}
          />
        );
        ...
        return (
          <div className="h-full relative">
            <QRScanner
              ...
              onClose={() => setView(user ? 'player_home' : 'landing')}
            />
            {/* Nessun popup locale: i popup sono gestiti globalmente in App.tsx */}
          </div>
        );
      
      case 'staff_home_mobile':
        return <HomeScreenMobile user={user} />;

      case 'privacy_consent':
        return (
          <PrivacyConsent
            lang={lang}
            onAccept={() => setView('player_home')}
            onDeny={() => setView('landing')}
            clubName={localRulesData?.name || localRulesData?.club || ""}
          />
        );
      ...
      case 'player_home':
        return (
          <PlayerHome
            ...
            onStartScanner={() => setView('qr_scanner')}
            // Added missing onStopRound property
            onStopRound={onStopRound}
            onShowLocalRules={() => {}}
            onShowCartRules={() => {}}
            isTrackingActive={roundStatus === 'started'}
      ...
      case 'player_monitor':
        if (user?.role === 'marshall' || isMobile) {
          return (
            <MarshallMobileView
              userId={user?.uid || ''}
              userDisplayName={user?.displayName || 'Marshall'}
      ...
      case 'admin_greenkeeping_workers':
        // Added missing user prop to fix error on line 547
        return (
          <WorkersListScreen
      ...
      case 'admin_greenkeeping_worker_create':
        return (
          <WorkerCreateScreen
            ...
            lang={lang}
            user={user}
            onBack={() => setView('admin_greenkeeping_workers')}
          />
        );
      ...
      case 'debug_dashboard':
        return (
          <DebugDashboard
            onBack={() =>
              setView(user?.role === 'sys_admin' ? 'super_admin_dashboard' : 'club_dashboard')
            }
            lang={lang}
          />
        );
